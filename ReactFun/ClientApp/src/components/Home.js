...
function Welcome () {
    return (<h1>Welcome to the ultimate March Madness website! Below is a list the need-to-know info of all teams participating in this years tournament.</h1>);
}

function TeamList() {
    return (
        <div>
            {teamsNCAA.map(team => <Team {...team} />)}
        </div>
        );
}
...
class Team extends React.Component {
    render() {
        ...
        return (
            <div>
                <h4>{team.school} {team.name} ({team.abbrev})</h4>
                <h6>Location: {team.city}, {team.state}</h6>
                <h6>Population: {team.pop}</h6>
            </div>
        );
    }
}
